...
function passwordmanager_manage_groups() {
    ...
    if(isset($_POST['action'])){
        if ($_POST['action'] === 'Add') {
            ...
            echo "User Has submitted the form to ADD a group : <strong> $name </strong>";

            //CHECK DUPLICATE
            //$check_duplicate = $wpdb->query($wpdb->prepare("SELECT * FROM " . TABLE_GRP . " WHERE name = '$name'"));
            $check_duplicate = $wpdb->query($wpdb->prepare("SELECT * FROM " . TABLE_GRP . " WHERE name = %s ",$name));
            if ($check_duplicate === 1) {
                require 'messages.php';
                alert_duplicate('group', 'duplicate', $name);
            } else {
                $insert_query = $wpdb->query($wpdb->prepare("INSERT INTO " . TABLE_GRP . " ( name, trash ) VALUES ( %s, %d )", $name, 0));
            }
            //
            //$insert_query = $wpdb->insert(TABLE_GRP, array('name' => $name));
        }
    }
    ...
    if(isset($_POST['action'])){
        if ($_POST['action'] === 'Update') {
            ...
            //$check_duplicate = $wpdb->query($wpdb->prepare("SELECT * FROM " . TABLE_GRP . " WHERE name = '$newvalue'"));
            $check_duplicate = $wpdb->query($wpdb->prepare("SELECT * FROM " . TABLE_GRP . " WHERE name = %s ", $newvalue));
            if ($check_duplicate === 1) {
                require 'messages.php';
                alert_duplicate('group', 'duplicate', $oldvalue);
            } else {
                $wpdb->update(
                        TABLE_GRP, array('name' => $newvalue), array('name' => $oldvalue), array('%s')
                );
    ...
    if(isset($_POST['Delete'])){
        if ($_POST['Delete'] === 'Delete') {
            ...
            $wpdb->update(
                    TABLE_GRP, array('trash' => '1'), array('name' => $oldvalue), array('%d')
            );
        }
    }
    ...
    <form name="group-edit" action="<?php echo str_replace('%7E', '~', $_SERVER['REQUEST_URI']); ?>" method="POST">
        <select name="oldvalue" id="mySelect" onchange="selectionchange();">
            <option value="">---</option>
    <?php
    $grouplists = $wpdb->get_results(" SELECT * FROM " . TABLE_GRP . " WHERE trash LIKE '0'");
    if ($grouplists) {
